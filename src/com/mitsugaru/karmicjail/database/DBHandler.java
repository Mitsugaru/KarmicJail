...
public class DBHandler extends AbstractModule {
    ...
    public boolean checkTables() {
        ...
        if(useMySQL) {
            ...
            mysql = new MySQL(plugin.getLogger(), "KJ", config.host,
    ...
    private void importSQL() {
        ...
        try {
            ...
            plugin.getLogger().log(Level.SEVERE,
                    KarmicJail.TAG + " SQL Exception on Import", e);
        } finally {
            cleanup(statementResult, null);
    ...
    public void cleanup(ResultSet rs, PreparedStatement statement) {
        if(statement != null) {
            try {
                ...
                plugin.getLogger().log(Level.SEVERE,
                        KarmicJail.TAG + " SQL Exception on cleanup.", e);
            }
        }
        if(rs != null) {
            try {
    ...
    public void resetPlayerInventory(String name) {
        JailLogic logic = plugin.getModuleForClass(JailLogic.class);
        int id = getPlayerId(name);
        if(id == -1) {
            // Unknown player?
