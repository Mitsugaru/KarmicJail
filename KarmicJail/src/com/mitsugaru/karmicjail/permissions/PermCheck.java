         ...
         if(pluginName.equals("PermissionsEx")) {
            for(PermissionGroup group : PermissionsEx.getPermissionManager().getGroups()) {
               try {
                  ...
                  this.plugin.getLogger().warning(KarmicJail.TAG + " Cannot grab default group.");
                  return def;
               }
            }
            ...
            def = ((GroupManager) this.plugin.getServer().getPluginManager().getPlugin("GroupManager")).getWorldsHolder()
                  .getWorldData(this.plugin.getServer().getWorlds().get(0).toString()).getDefaultGroup().getName();
         }
      }
      return def;
   }
