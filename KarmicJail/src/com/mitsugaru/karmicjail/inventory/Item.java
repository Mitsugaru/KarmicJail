...
public class Item extends MaterialData {
   ...
   public boolean areSame(Object obj) {
      // Both blocks
      try {
         if(this.getItemType().isBlock() && ((Item) obj).getItemType().isBlock()) {
         ...
         } else if(!this.getItemType().isBlock() && !((Item) obj).getItemType().isBlock()) {
   ...
   public static boolean isTool(int id) {
      final int[] tool = { 256, 257, 258, 259, 261, 267, 268, 269, 270, 271, 272, 273, 274, 275, 276, 277, 278, 279, 283, 284, 285, 286, 290, 291,
   ...
   public static String getItemEnchantments(ItemStack item) {
      ...
      final TreeSet<ComparableEnchantment> keys = new TreeSet<ComparableEnchantment>(map.keySet());
      for(ComparableEnchantment key : keys) {
         sb.append(key.getId() + "v" + item.getEnchantments().get(key.getEnchantment()).intValue() + "i");
      }
   ...
   public static ItemStack addEnchantments(String enchant, ItemStack item) {
      ...
      for(int s = 0; s < cut.length; s++) {
         try {
            ...
            final EnchantmentWrapper e = new EnchantmentWrapper(Integer.parseInt(cutter[0]));
            item.addUnsafeEnchantment(e.getEnchantment(), Integer.parseInt(cutter[1]));
